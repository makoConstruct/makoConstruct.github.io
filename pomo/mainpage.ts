...
//configuration
var config:{startingDate:number, onTime:number, offTime:number, lightMode:boolean} = {
...
var readUrlDataToConfig = ()=> {
	;(getEl('shareLinkDisplay') as HTMLInputElement).value = document.location.href
	var lhash = location.hash
	if(lhash != ""){
		var urlDataArray = lhash.substr(1).split(sep)
		var minutesRead = (mr)=> {
			var ml = mr.split(':')
			var emptyIsZero = (ns)=> ns == "" ? 0 : parseFloat(ns)
...
var urlFor = (key)=> 'http://'+document.location.pathname+key
...
function configurationChanged(){
	...
	;(getEl('shareLinkDisplay') as HTMLInputElement).value = newUrl
